...
### Change to introduce the classes labels during training
    def distance_between_mean_class(self):
        proto_to_proto = list_of_distance(self.mean_class, self.mean_class)
        proto_to_proto_non_zero = proto_to_proto.clone()
        proto_to_proto_non_zero[proto_to_proto==0] = 1e+6
        return proto_to_proto_non_zero
    ...
    def forward(self, x, labels):
        ...
        self.kl = self.kl
        #define reulator
        self.R_1 = torch.mean(torch.min(list_of_distance(self.mean_class, z), dim=1).values)
        self.R_2 = torch.mean(torch.min(self.laten_similarity, dim=1).values)
        self.R_3 = torch.mean(torch.min(self.distance_between_mean_class(), dim=1).values)
        return z
...
class CondVAELoss(nn.Module):
    ...
    """
        kld_weight: float
        model: CondVAEBuilder
        The reduction factor of the Kullback-Leibler divergence
    """
    def forward(self, model,prediction,target, input, input_decoded,  kld_weight=0.0025, alpha_R1 = 0.5, alpha_R2 = 0.5, alpha_R3=0.02,):
        classif_loss = F.nll_loss(prediction, target)
        reconst_error = F.mse_loss(input_decoded, input)
        R_1 = model.vae.encoder.R_1 * alpha_R1
        R_2 = model.vae.encoder.R_2 * alpha_R2 
        R_3 = (1/( model.vae.encoder.R_3+ 1)) * alpha_R3
